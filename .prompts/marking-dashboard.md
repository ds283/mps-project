...
- show a clear label to indicate whether a moderation event has been triggered
...
- Each `MarkingEvent` should be a separate worksheet within the Excel report. Collect all `MarkingWorkflow` instances
  for the same `MarkingEvent` on a single worksheet, but ensure they are clearly separate visually. The excel report
  should be generated by a background Celery workflow.
