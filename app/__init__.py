...
def read_configuration(app: Flask, config_name: str):
    ...
    app.config.from_pyfile("language_analysis.py")
    app.config.from_pyfile("similarity_analysis.py")
...
def create_app():
    ...
    tasks.register_ticket_notification_tasks(celery)
    ...
    tasks.register_custom_offer_hint_tasks(celery)
    ...
    tasks.register_canvas_push_tasks(celery)
    tasks.register_consent_tasks(celery)
    ...
    def latextomarkdown(latex_string):
        if latex_string is None:
            return (
                r'<div class="alert alert-danger">An empty string was supplied. '
