...
{% if template is not none %}
    <div class="d-flex flex-column justify-content-start align-items-start gap-2">
        <div class="d-flex flex-row flex-wrap justify-content-start align-items-center gap-2">
            ...
            {% endif %}
            {% if pclass is defined and pclass is not none %}
            ...
            {% endif %}
            {% if tenant is defined and tenant is not none %}
                <span class="badge bg-warning text-dark ms-1" data-bs-toggle="tooltip" title="Override for tenant: {{ tenant.name }}">
...
def _build_pclass_menu_templ() -> Template:
    env: Environment = current_app.jinja_env
    return env.from_string(_email_pclass_template_menu)


def _build_tenant_menu_templ() -> Template:
    env: Environment = current_app.jinja_env
    return env.from_string(_email_tenant_template_menu)


def template_data_pclass_override(pclass: ProjectClass, template_list: List):
    """
...
def template_data_tenant_override(tenant: Tenant, template_list: List):
    """
    Generate DataTables data for email templates.
    ...
    def _process(template_type: int, template):
        ...
        return {
            ...
            "subject": render_template(subject_templ, template=template),
            "version": render_template(version_templ, template=template),
            "status": render_template(status_templ, template=template, tenant=tenant),
            "comment": render_template(comment_templ, template=template),
            "menu": render_template(
                menu_templ,
                template=template,
