...
{% for e in f.enrollments %}
    {% if e.pclass.publish and (e.pclass.uses_marker or e.pclass.uses_presentations) %}
        ...
        {% elif e.marker_state == e.MARKER_SABBATICAL and e.presentations_state == e.PRESENTATIONS_SABBATICAL %}
            <{% if not disabled %}a{% else %}span{% endif%} class="label label-default" {% if not disabled %}href="{{ url_for('manage_users.edit_enrollment', id=e.id, url=url) }}"{% endif %}><i class="fa fa-times"></i> {{ e.pclass.abbreviation }} (sabbat)<{% if not disabled %}/a{% else %}/span{% endif %}>
        ...
        %}
            <{% if not disabled %}a{% else %}span{% endif%} class="label label-default" {% if not disabled %}href="{{ url_for('manage_users.edit_enrollment', id=e.id, url=url) }}"{% endif %}><i class="fa fa-times"></i> {{ e.pclass.abbreviation }} (exempt)<{% if not disabled %}/a{% else %}/span{% endif %}>
        {% else %}
            <{% if not disabled %}a{% else %}span{% endif%} class="label label-danger" {% if not disabled %}href="{{ url_for('manage_users.edit_enrollment', id=e.id, url=url) }}"{% endif %}><i class="fa fa-exclamation-triangle"></i> {{ e.pclass.abbreviation }} (unknown)<{% if not disabled %}/a{% else %}/span{% endif %}>
        {% endif %}
    {% endif %}
{% endfor %}
{% if ns.count == 0 %}
