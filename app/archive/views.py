...
def reports():
    ...
    if pclass_filter is not None and pclass_filter != "all":
        ...
        if flag:
            pclass: ProjectClass = (
                db.session.query(ProjectClass).filter_by(id=value).first()
            )
            if pclass is None:
                pclass_filter = "all"
            elif (
                    not current_user.has_role("root")
                    and pclass.tenant_id not in allowed_tenant_ids
            ):
                pclass_filter = "all"
        else:
            pclass_filter = "all"
    ...
    if group_filter is not None and group_filter != "all":
        ...
        if flag:
            group: ResearchGroup = (
                db.session.query(ResearchGroup).filter_by(id=value).first()
            )
            if group is None:
                group_filter = "all"
...
def reports_ajax():
    ...
    if group_filter is not None and group_filter != "all":
        ...
        if flag:
            base_query = (
                base_query.join(
                    SubmissionRecord, SubmissionRecord.owner_id == SubmittingStudent.id
                )
