...
def ProjectMixinFactory(convenor_editing: bool, uses_tags: bool, uses_research_groups: bool, project_classes_qf, group_qf):
    class ProjectMixin:
            ...
            def validate_group(form, field):
                for pclass in form.project_classes.data:
                    ...
                    if field.data is None:
                        raise ValidationError(
                            "Projects attached to class '{cl}' are advertised by affiliation/research group. "
        ...
        project_classes = QuerySelectMultipleField(
            "For which project types do you wish to offer this project?",
            query_factory=project_classes_qf,
            get_label="name",
        ...
        dont_clash_presentations = BooleanField(
            "Prevent co-scheduling presentation with multiple students taking the same project",
            default=True,
...
def AddDescriptionFormFactory(project_id):
    ...
    class AddDescriptionForm(Form, Mixin):
        label = StringField(
            ...
            description="Enter a short label to identify this description in the list. The label will not be visible to students.",
        )
