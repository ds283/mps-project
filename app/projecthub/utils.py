...
def validate_project_hub(record: SubmissionRecord, user: User, message=False):
    ...
    if user.has_role('office') or user.has_role('moderator') or user.has_role('exam_board') \
    ...
    if message:
        if record.project is not None:
            flash('You are not currently authorized to view the project hub for '
                  'student "{name}" (project "{title}")'.format(name=record.owner.student.user.name,
                                                                title=record.project.title), 'info')
        else:
            flash('You are not currently authorized to view the project hub for '
                  'student "{name}"'.format(name=record.owner.student.user.name))
