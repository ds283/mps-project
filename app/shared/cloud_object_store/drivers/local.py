...
def _check_is_object(prefix: Path, leaf: Path) -> None:
    ...
    if not abs_path.is_file():
        raise RuntimeError('cloud_object_store: asset with specified key="{key}" is not a stored object'.format(key=leaf))
...
def _check_not_exists(prefix: Path, leaf: Path) -> None:
    ...
    if abs_path.exists():
        raise RuntimeError('cloud_object_store: an asset with key="{key}" already exists in the store'.format(key=leaf))
...
class LocalFileSystemDriver:
    def __init__(self, uri: SplitResult, data: Dict):
        if uri.hostname is not None and len(uri.hostname) > 0:
            print("cloud_object_store: hostname is not supported for file:// handler (hostname={name})".format(name=uri.hostname))

        self._root: Path = Path(uri.path).resolve()

        if self._root.is_file():
            raise RuntimeError("cloud_object_store: supplied file:// root is a file, not a directory ({path})".format(path=self._root))
    ...
    def get_url(self, key: Path) -> str:
        raise NotImplementedError("The get_url() method is not supported for the LocalFileSystem driver")
