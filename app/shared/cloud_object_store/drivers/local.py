...
def _check_is_object(prefix: Path, leaf: Path) -> None:
    ...
    if not abs_path.exists():
        raise RuntimeError(
            'cloud_object_store: could not find asset with specified key="{key}"'.format(
                key=leaf
            )
        )
    ...
    if not abs_path.is_file():
        raise RuntimeError(
            'cloud_object_store: asset with specified key="{key}" is not a stored '
...
def _check_prefix_ok(prefix: Path, leaf: Path) -> None:
    ...
    if abs_path.is_file():
        raise RuntimeError(
            'cloud_object_store: prefix="{prefix}" conflicts with an asset key'.format(
                prefix=leaf
            )
        )
...
def _check_not_exists(prefix: Path, leaf: Path) -> None:
    ...
    if abs_path.exists():
        raise RuntimeError(
            'cloud_object_store: an asset with key="{key}" already exists in the '
...
class LocalFileSystemDriver:
    def __init__(self, uri: SplitResult, data: Dict):
        if uri.hostname is not None and len(uri.hostname) > 0:
            print(
                "cloud_object_store: hostname is not supported for file:// handler (hostname={name})".format(
                    name=uri.hostname
                )
            )
        ...
        if self._root.is_file():
            raise RuntimeError(
                "cloud_object_store: supplied file:// root is a file, not a directory ({path})".format(
                    path=self._root
                )
            )
