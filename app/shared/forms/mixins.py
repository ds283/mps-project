...
class DefaultLicenseMixin:
    default_license = QuerySelectField(
        "Default license for content I upload",
...
def FacultyDataMixinFactory(admin=False, enable_canvas=False):
    class FacultyDataMixin:
        ...
        sign_off_students = BooleanField(
            "Enforce confirmation requests",
            default=True,
        ...
        project_capacity = IntegerField(
            "Default project capacity",
            description="Default maximum number of students that can be assigned to a project",
            validators=[NotOptionalIf("enforce_capacity")],
        )

        show_popularity = BooleanField(
        ...
        dont_clash_presentations = BooleanField(
            "Don't schedule presentations with other students taking the same project",
            default=True,
        ...
        if admin:
            CATS_supervision = IntegerField(
                "Guideline number of CATS available for project supervision",
            ...
            CATS_presentation = IntegerField(
                "Guideline number of CATS available for presentation assessment",
                description="Leave blank for default assignment",
                validators=[Optional()],
            )
...
class PeriodPresentationsMixin:
    ...
    number_assessors = IntegerField(
        ...
        default=2,
        description="Enter the number of faculty assessors scheduled per group during the presentation assessment",
    ...
    max_group_size = IntegerField(
        "Maximum group size",
        default=5,
        description="Enter the desired maximum group size. Some groups may be smaller if this is required.",
        validators=[NotOptionalIf("has_presentation")],
    )
