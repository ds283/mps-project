...
def register_assessment_tasks(celery):
    ...
    def adjust_submitter(self, submitter_id, current_year):
        self.update_state(
            state="STARTED",
            meta={
                "msg": "Looking up SubmittingStudent record for id={id}".format(
                    id=submitter_id
                )
            },
        )
        ...
        try:
            submitter = (
                db.session.query(SubmittingStudent).filter_by(id=submitter_id).first()
            )
        ...
        assessments = (
            ...
            .filter_by(year=current_year, feedback_open=True)
            .all()
        )
        ...
        for assessment in assessments:
            for rec in submitter.records:
                if (
                    get_count(assessment.submission_periods.filter_by(id=rec.period_id))
                    > 0
                ):
                    if (
                        get_count(
                            assessment.submitter_list.filter_by(submitter_id=rec.id)
                        )
                        == 0
                    ):
                        data = SubmitterAttendanceData(
                            submitter_id=rec.id,
