...
def _get_course_code(row, current_line) -> DegreeProgramme:
    ...
    if "course code" in row:
        ...
        if course_name in course_map:
            ...
            print('## course name "{name}" not found in look-up table at row {row}'.format(name=course_name, row=current_line))
...
def _guess_year_data(
    ...
    if estimated_year_of_course < 0:
        print(
            "!! ERROR: estimated year of course is negative: current_year={cy}, cohort={ch}, FY={fy}".format(
                cy=current_year, ch=cohort, fy=fyear_shift
            )
        )
    ...
    if difference < 0:
        ...
        if programme.foundation_year and difference == -1:
            ...
            reason = f"student's estimated year of course Y{estimated_year_of_course} was earlier than the imported value = Y{year_of_course}"
            if programme.year_out:
                reason += f" (note: this student belongs to programme '{programme.full_name}' which has a year out in Y{programme.year_out_value})"
            raise StudentSkipRow(current_line, reason=reason)
...
def _match_existing_student(current_line, username, email) -> Optional[Tuple[bool, StudentData]]:
    ...
    if existing_record is not None:
        if not existing_record.has_role("student"):
            ...
            raise StudentSkipRow(
                current_line,
                reason=f"imported values matched to an existing user '{existing_record.name}' that is not a student",
            )
    ...
    if existing_record is not None:
        if not existing_record.has_role("faculty"):
            ...
            raise FacultySkipRow(
                current_line,
                reason=f"imported values matched to an existing user '{existing_record.name}' that is not a faculty member",
            )
...
def register_batch_create_tasks(celery):
    ...
    def import_faculty_finalize(self, result_data, record_id, user_id):
        ...
        user.post_message(
            "Faculty account import complete: {created} created, {merged} merged, {errors} errors, {failed} failed, {ignored} ignored".format(
                created=num_created,
                merged=num_merged,
                failed=num_failed,
                ignored=num_ignored,
    ...
    def import_student_finalize(self, result_data, record_id, user_id):
        ...
        user.post_message(
            "Student account import complete: {created} created, {merged} merged, {errors} errors, {failed} failed, {ignored} ignored".format(
                created=num_created,
                merged=num_merged,
                failed=num_failed,
                ignored=num_ignored,
