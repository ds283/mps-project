...
def register_canvas_tasks(celery):
    ...
    def canvas_submission_checkin_module(self, pid, API_root: str):
            ...
            if sub["workflow_state"] != "unsubmitted":
                ...
                if num_student == 1:
                    ...
                    if record is not None:
                        print(
                            '** [{pcl}]: Student "{name}" with email address "{email}" has a Canvas submission available'.format(
                                pcl=config.name, name=sd.user.name, email=sd.user.email
                            )
                        )
                        record.canvas_submission_available = True
                    else:
                        print(
                            '** [{pcl}]: Submission record for student "{name}" with email address "{name}" is None'.format(
                                pcl=config.name, name=sd.user.name, email=sd.user.email
                            )
                        )

                elif num_student > 1:
                    msg = "** [{pcl}]: Canvas user with userid {uid} matches multiple (N={n}) submitting student records".format(
                        pcl=config.name, uid=canvas_id, n=num_student
                    )
                    print(msg)
                    current_app.logger.warning(msg)
        ...
        try:
            ...
            msg = 'Could not synchronize submission availability with Canvas for project "{pname}" because of a database error'.format(
                pname=config.name
            )
            print(msg)
            current_app.logger.error(msg)
    ...
    def pull_report(self, rid, user_id):
        ...
        if not period.canvas_enabled:
            if user is not None:
                user.post_message(
                    "Can not pull report from Canvas for submitter {name} because Canvas "
        ...
        if submitter.canvas_user_id is None:
            if user is not None:
                user.post_message(
                    "Can not pull report from Canvas for submitter {name} because this record "
        ...
        if len(attachments) == 0:
            ...
            if user is not None:
                user.post_message(
                    "More than one attachment is present in the Canvas record for submitter {name}. "
        ...
        print(
            '** [Canvas, {pcl}]: Downloading attachment "{file}" for submitting student {name}'.format(
                pcl=config.name, file=attachment["id"], name=submitter.student.user.name
            )
        )
