    ...
    def send_api_events_to_telemetry(self):
        ...
        with ScratchFileManager(suffix=".csv") as csv_scratch:
            ...
            if not path.exists(csv_path) or not path.isfile(csv_path):
                self.update_state(state="FAILURE", meta={"msg": "Extraction of Cloud API backend data to CSV file did not produce any usable output"})
                raise self.retry()
            ...
            with ScratchFileManager(suffix=".tar.gz") as archive_scratch:
                ...
                if not path.exists(archive_path) or not path.isfile(archive_path):
                    self.update_state(
                        state="FAILURE", meta={"msg": "Compression of extracted Cloud API backend data did not produce any usable output"}
                    )
                    raise self.retry()
                ...
                with open(archive_path, "rb") as f:
                    _ = object_store.put(
                        tgz_key, audit_data="send_cloud_api_events_to_telemetry", data=BytesIO(f.read()), mimetype="application/gzip"
                    )
