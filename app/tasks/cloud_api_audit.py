...
def register_cloud_api_audit_tasks(celery):
    ...
    def send_api_events_to_telemetry(self):
        self.update_state(
            state="STARTED",
            meta={"msg": "Initiating deposit to cloud API audit events to telemetry"},
        )
        ...
        if rows == 0:
            self.update_state(
                state="SUCCESS",
                meta={"msg": "No audit records were returned from Cloud API backend"},
            )
            return True
        ...
        with ScratchFileManager(suffix=".csv") as csv_scratch:
            ...
            with ScratchFileManager(suffix=".tar.gz") as archive_scratch:
                ...
                self.update_state(
                    "PROGRESS",
                    meta={"msg": "Uploading compressed CSV to telemetry object store"},
                )

                with open(archive_path, "rb") as f:
                    _ = object_store.put(
                        tgz_key,
                        audit_data="send_cloud_api_events_to_telemetry",
                        data=BytesIO(f.read()),
        ...
        self.update_state(
            "PROGRESS",
            meta={"msg": "Requesting backend to remove records sent to telemetry"},
        )
