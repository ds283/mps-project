...
def register_precompute_tasks(celery):
    ...
    def project_approval(self, current_user_id):
        try:
            data = db.session.query(ProjectDescription).filter(ProjectDescription.workflow_state == WorkflowMixin.WORKFLOW_APPROVAL_QUEUED).all()
        except SQLAlchemyError as e:
            current_app.logger.exception("SQLAlchemyError exception", exc_info=e)
            raise self.retry()
    ...
    def project_rejected(self, current_user_id):
        try:
            data = db.session.query(ProjectDescription).filter(ProjectDescription.workflow_state == WorkflowMixin.WORKFLOW_APPROVAL_REJECTED).all()
        except SQLAlchemyError as e:
            current_app.logger.exception("SQLAlchemyError exception", exc_info=e)
            raise self.retry()
    ...
    def assessor_data(self, current_user_id):
        ...
        try:
            projects = db.session.query(Project).join(User, User.id == Project.owner_id).filter(User.active == True).all()
        except SQLAlchemyError as e:
            current_app.logger.exception("SQLAlchemyError exception", exc_info=e)
            raise self.retry()
    ...
    def workload_data(self):
        ...
        try:
            data = db.session.query(FacultyData).join(User, User.id == FacultyData.id).filter(User.active == True).all()
        except SQLAlchemyError as e:
            current_app.logger.exception("SQLAlchemyError exception", exc_info=e)
            raise self.retry()
