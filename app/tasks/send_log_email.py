...
def register_send_log_email(celery, mail: Mail):
    ...
    def send_email_live(self, task_id, msg: EmailMessage):
        ...
        progress_update(
            task_id, TaskRecord.RUNNING, 40, "Sending email...", autocommit=True
        )
        try:
            ...
            if hasattr(msg, "body") and msg.body is not None:
                ...
                current_app.logger.error(
                    "-- send_mail() ignored attempt to send email with empty body"
                )
            ...
            current_app.logger.info(
                '**   from_email = "{email}"'.format(email=from_email)
            )
            current_app.logger.info(
                '**   recipients = "{recipients}"'.format(recipients=recipients)
            )
    ...
    def send_email_to_console(self, task_id, msg: EmailMessage):
        progress_update(
            ...
            TaskRecord.RUNNING,
            40,
            "Logging email message to the console...",
            autocommit=True,
        )
    ...
    def log_email(self, result_data, task_id, msg: EmailMessage):
        ...
        return {"outcome": "success", "key": log.id}
    ...
    def email_success(result_data, task_id):
        if "outcome" not in result_data:
            progress_update(
                ...
                TaskRecord.FAILURE,
                100,
                "Task failed with no outcome reported",
                autocommit=True,
            )
            return
        ...
        if outcome == "unknown":
            progress_update(
                ...
                TaskRecord.FAILURE,
                100,
                "Task failed with unknown outcome",
                autocommit=True,
            )
            return
        ...
        if outcome == "failure":
            progress_update(
                task_id, TaskRecord.FAILURE, 100, "Task failed", autocommit=True
            )
            return

        progress_update(
            task_id, TaskRecord.SUCCESS, 100, "Task complete", autocommit=True
        )
    ...
    def email_failure(task_id):
        progress_update(
            task_id, TaskRecord.FAILURE, 1000, "Task failed", autocommit=True
        )
