...
def register_services_tasks(celery):
    ...
    def send_distribution_list(self, list_ids, notify_addresses, subject, body, reply_to, user_id):
        ...
        if isinstance(notify_addresses, list) and len(notify_addresses) > 0:
            notify = group(send_notify.s(x, subject, body, reply_to, workflow_id) for x in notify_addresses)
            work = work | notify
    ...
    def send_user_record(self, user_id, subject, body, reply_to, workflow_id):
        ...
        if workflow is None:
            self.update_state("FAILURE", meta={"msg": "Could not load email workflow"})
            raise KeyError("EmailWorkflow record corresponding to id={num} is missing".format(num=workflow_id))
    ...
    def send_email_list(self, to_addresses, notify_addresses, subject, body, reply_to, user_id):
        ...
        work = group(send_email_addr.s(x, subject, body, reply_to, workflow_id) for x in to_addresses)
    ...
    def email_success(self, prior_result, subject, user_id):
        ...
        record.post_message(
            'Email with subject "{subj}" successfully sent to all recipients'.format(subj=subject),
            "success",
            autocommit=True,
        )
