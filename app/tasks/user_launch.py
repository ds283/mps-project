...
def register_user_launch_tasks(celery):
    ...
    def mark_user_task_started(task_id, name):
        progress_update(
            ...
            TaskRecord.RUNNING,
            0,
            'Starting task "{name}"'.format(name=name),
            autocommit=True,
        )
    ...
    def mark_user_task_ended(self, task_id, name, user_id, notify=False):
        progress_update(
            ...
            TaskRecord.SUCCESS,
            100,
            'Task "{name}" complete'.format(name=name),
        ...
        if notify:
            owner.post_message(
                'Task "{name}" completed successfully'.format(name=name),
                "success",
                autocommit=True,
            )
    ...
    def mark_user_task_failed(self, task_id, name, user_id):
        progress_update(
            ...
            TaskRecord.FAILURE,
            100,
            "Error while running task '{name}'".format(name=name),
        )
        ...
        owner.post_message(
            'Task "{name}" failed to complete'.format(name=name),
            "danger",
            autocommit=True,
        )
