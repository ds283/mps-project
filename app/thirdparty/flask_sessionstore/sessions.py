...
class MongoDBSessionInterface(SessionInterface):
    ...
    # set up a new session object
    def open_session(self, app, request):
        # get session ID from cookie
        sid = request.cookies.get(app.session_cookie_name)

        # no session ID recovered, so a new one needs to be generated
        if not sid:
            sid = self._generate_sid()
            return self.session_class(sid=sid, permanent=self.permanent)

        # if a session ID was recovered, need to recover session data from the database

        # if we are signing the cookie then decode the sid and check its signature;
        # if it fails, then generate a new session and a new session ID
        if self.use_signer:
            signer = self._get_signer(app)
            if signer is None:
                return None
        ...
        # a document was not recovered: return a blank session object
        return self.session_class(sid=sid, permanent=self.permanent)
