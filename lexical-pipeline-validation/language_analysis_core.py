...
NLL_ENABLED       — False; NLL computation via /tokenize + /score is not
                    supported.  mean_nll / nll_cv are always None.
...
def analyse_paper_from_text(
    ...
    *llama_server_url* is accepted for API compatibility but NLL computation
    is no longer supported (NLL_ENABLED = False).  mean_nll and nll_cv are
    always None.
