...
rc = page.insert_text(p6, 'Created at {date}'.format(date=creation_date.strftime("%a %d %b %Y %H:%M:%S")), color=black, fontname='Helvetica', fontsize=12)

x0 = int(w/2)
p7 = fitz.Point(x0, y0)

rc = page.insert_text(p7, 'Last modified: {date}'.format(date=modified_date.strftime("%a %d %b %Y %H:%M:%S")), color=black, fontname='Helvetica', fontsize=12)
ytop = ytop + 12 + 8
